...
public class ProgrammUpdateSuchen {
			...
			SwingUtilities.invokeLater(() -> {
				...
				if (progInfo.getVersion().toNumber() == 0) {
					...
					if (checkForNewerVersion(progInfo.getVersion())) {
						...
						JOptionPane.showMessageDialog(null, "Sie benutzen die neueste Version von Mediathog.", UPDATE_SEARCH_TITLE, JOptionPane.INFORMATION_MESSAGE);
					}
				}
			});
		}
	...
	*/
	private Optional<ServerProgramInformation> retrieveProgramInformation() {
		...
		final Request request = new Request.Builder().url(killPver).get().build();
