...
public class FilmeLaden {
	...
	private final tList diffListe = new tList();
	...
	*/
	private boolean hasNewRemoteFilmlist() {
		...
		if (GraphicsEnvironment.isHeadless() || GuiFunktionen.getImportArtFilme() == setFilmsAuto)
		showDialogs = false;
	...
	*/
	private boolean performUpdateCheck(tList listeFilme, String dateiUrl) {
		boolean result = true;
	...
	*/
	...
	public boolean loadFilmlist(String dateiUrl, boolean immerNeuLaden) {
		...
		tList listeFilme = daten.getListeFilme();
	...
	private void undEnde(ListenerFilmeLadenEvent event) {
		...
		if (!diffListe.isEmpty()) {
			...
			final tList listeFilme = daten.getListeFilme();
	...
	private void fillHash(tList listeFilme) {
	...
	*/
	private void findAndMarkNewFilms(tList listeFilme) {
